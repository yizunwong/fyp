...
import { getToken, saveToken, saveRefreshToken, getRefreshToken } from '@/lib/auth';
...
export const customFetcher = async <T = any>({
  ...
  if (typeof url === 'string' && (url.endsWith('/auth/login') || url.endsWith('/auth/refresh'))) {
    if ((json as any)?.access_token) await saveToken((json as any).access_token);
    if ((json as any)?.refresh_token) await saveRefreshToken((json as any).refresh_token);
  }
