...
const registrationStats = [
  ...
  { label: "On-Chain", value: 46, icon: Shield, color: "#047857", bg: "bg-emerald-50" },
  { label: "Approved", value: 132, icon: TrendingUp, color: "#1d4ed8", bg: "bg-blue-50" },
  { label: "Docs Required", value: 9, icon: FileText, color: "#4338ca", bg: "bg-indigo-50" },
];

const recentRegistrations = [
  { id: "FARM-145", farmer: "Nur Aina", state: "Perak", status: "pending_review", submitted: "2h ago" },
  { id: "FARM-142", farmer: "Hafiz Rahman", state: "Johor", status: "approved", submitted: "6h ago" },
  { id: "FARM-139", farmer: "Lim Wei", state: "Penang", status: "docs_required", submitted: "1d ago" },
];

const pendingClaims = [
  { id: "SUB-2025-0188", policy: "Flood Relief Fund", amount: "RM 5,000", state: "Terengganu", type: "oracle" },
  { id: "SUB-2025-0187", policy: "Organic Farming Support", amount: "RM 3,500", state: "Pahang", type: "manual" },
  { id: "SUB-2025-0186", policy: "Drought Assistance", amount: "RM 4,200", state: "Kedah", type: "oracle" },
];

const activePolicies = [
  { name: "Flood Damage Compensation", type: "Flood", activeTo: "Dec 2025", status: "active" },
  { name: "Organic Farming Incentive", type: "Manual", activeTo: "Dec 2025", status: "draft" },
  { name: "Drought Relief Subsidy", type: "Drought", activeTo: "Oct 2025", status: "active" },
];

const weatherAlerts = [
  { region: "Kuala Terengganu", severity: "Critical", message: "Heavy rainfall expected in next 12h" },
  { region: "Kota Bharu", severity: "Warning", message: "River levels rising, monitor closely" },
];

function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { text: string; className: string }> = {
    pending_review: { text: "Pending", className: "bg-yellow-100 text-yellow-700" },
    approved: { text: "Approved", className: "bg-green-100 text-green-700" },
    docs_required: { text: "Docs Required", className: "bg-blue-100 text-blue-700" },
    draft: { text: "Draft", className: "bg-gray-100 text-gray-700" },
    active: { text: "Active", className: "bg-emerald-100 text-emerald-700" },
  };
  const data = map[status] ?? { text: status, className: "bg-gray-100 text-gray-700" };
  return (
...
export default function AgencyDashboardScreen() {
  ...
  useAgencyLayout({
    ...
    subtitle: "Overview of registrations, policies, and claims",
  });
          ...
          return (
            ...
            >
              <View className="flex-row items-center justify-between mb-2">
                <View className={`w-10 h-10 ${item.bg} rounded-lg items-center justify-center`}>
  ...
  const pageContent = (
    <View className="px-6 py-6">
            ...
            {pendingClaims.map((claim) => (
              ...
              >
                <View className="flex-row items-center justify-between mb-1">
                  <Text className="text-gray-900 font-semibold">{claim.id}</Text>
                  <Text className="text-gray-500 text-xs">{claim.type}</Text>
                </View>
                <Text className="text-gray-700 text-sm">{claim.policy}</Text>
      ...
      <View className={isDesktop ? "flex-row gap-4 mt-4" : "gap-4 mt-4"}>
        <View className="flex-1 bg-white rounded-xl border border-gray-200 p-4">
          <View className="flex-row items-center justify-between mb-3">
            <Text className="text-gray-900 text-lg font-bold">Active Policies</Text>
            <TouchableOpacity
              onPress={() => router.push("/dashboard/agency/policies" as never)}
              className="px-3 py-1.5 rounded-lg bg-blue-50 border border-blue-200"
            >
              <Text className="text-blue-700 text-xs font-semibold">Manage</Text>
            </TouchableOpacity>
          </View>
          <View className="gap-3">
              ...
              >
                <View className="flex-1">
                  ...
                  <Text className="text-gray-600 text-xs">
                    {policy.type} • Active until {policy.activeTo}
                  </Text>
                </View>
        ...
        <View className="flex-1 bg-white rounded-xl border border-gray-200 p-4">
          <View className="flex-row items-center justify-between mb-3">
            <Text className="text-gray-900 text-lg font-bold">Weather Alerts</Text>
            ...
            {weatherAlerts.map((alert, idx) => (
              ...
              >
                ...
                <View className="flex-row items-center gap-1 mt-2">
                  ...
                  <Text className="text-gray-500 text-xs">Updated moments ago</Text>
                </View>
              </View>
            ))}
          </View>
